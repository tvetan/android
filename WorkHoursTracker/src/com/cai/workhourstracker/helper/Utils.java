...
public class Utils {
	...
	private static List<EntriesGroupViewRow> createEntiesGroupViewRow(
		...
		for (String key : groupedByJob.keySet()) {
			EntriesGroupViewRow entriesGroupViewRow = new EntriesGroupViewRow(key,
					groupedByJob.get(key));
	...
	public static String dateToMonthDateYearFormat(String dateAsString) {
		SimpleDateFormat yearMonthDateFormat = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss",
				Locale.getDefault());
		SimpleDateFormat monthDateYearFormat = new SimpleDateFormat("MMMM dd, yyyy",
				Locale.getDefault());
		try {
